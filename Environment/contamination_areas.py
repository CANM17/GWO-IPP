...
class DetectContaminationAreas():
    ...
    def areas_levels(self, mu, X_test, number_points, data_mu=False):

        dict_ = {}
        coord = copy.copy(X_test)
        dict_coord_ = {}
        dict_impor_ = {}
        dict_index_ = {}
        dict_limits = {}
        ...
        if data_mu:
            action_zone_data = [True] * center_peaks.shape[0]
        else:
            action_zone_data = [False] * center_peaks.shape[0]
        for w in range(len(array_max_x)):
            ...
            impo -= 10
            j += 1
        return center_peaks, action_zone_data
